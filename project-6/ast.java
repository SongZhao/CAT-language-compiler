...
class ProgramNode extends ASTnode {
    ...
    public void nameAnalysis() {
        ...
        SemSym Sym = symTab.lookupGlobal("main");
        if(Sym == null || !Sym.getType().isFnType())
        {
        	ErrMsg.fatal(0,0, "No main funtion");
        }
    }
...
class FormalsListNode extends ASTnode {
    ...
    public void CodeGen()
    {
    	for (FormalDeclNode fn : myFormals)
    		fn.CodeGen();
    }
    /**
...
class FnBodyNode extends ASTnode {
    ...
    public void CodeGen(String r)
    {
    	myStmtList.CodeGen(r);
    }
...
class VarDeclNode extends DeclNode {
    ...
    public SemSym nameAnalysis(SymTable symTab, SymTable globalTab) {
        ...
        int offset = symTab.getOffset();
        sym.setOffset(offset);
	System.out.println("In VD offset is " + offset);
	System.out.println("VD " + sym.getOffset());
        offset -= 4;
        symTab.recordOffest(offset);
	int offset1 = symTab.getOffset();
        //sym.setOffset(offset);
	System.out.println("In VD offset is?????? " + offset1);
	System.out.println("VD???? " + sym.getOffset());
        return sym;
    }    
...
class FnDeclNode extends DeclNode {
    ...
    public SemSym nameAnalysis(SymTable symTab) {
        ...
        symTab.recordOffest(0);
        // process the formals
        List<Type> typeList = myFormalsList.nameAnalysis(symTab);
        if (sym != null) {
            sym.addFormals(typeList);
        }
	symTab.recordOffest(symTab.getOffset() - 8);
	...
	if(localsCount != 0)    	
		Codegen.generate("subu", "$sp", "$sp", localsCount * 4);

    	//exit   //TODO
    	String fexit = "_"+myId.name()+"_Exit";
    	myBody.CodeGen(fexit);

    	Codegen.genLabel(fexit);
    	Codegen.generateIndexed("lw", "$ra", "$fp", -4 * parameterCount);
    	Codegen.generate("move", "$t0", "$fp");
    	Codegen.generateIndexed("lw", "$fp", "$fp", ((-4 * parameterCount) - 4));
...
abstract class StmtNode extends ASTnode {
    ...
    abstract public void CodeGen(String r);
}
...
class PostIncStmtNode extends StmtNode {
    ...
    public void CodeGen(String r)
    {
    	...
    	Codegen.genPush("$t0");
    	//TODO
    	Codegen.generateIndexed("lw","$t0","$sp",4,"PostInc--"); 
    	SemSym sym = ((IdNode)myExp).sym();
    	if(sym.isGlobal())
    		Codegen.generate("sw","$t0", "_"+((IdNode)myExp).name());
    	else
    		Codegen.generateIndexed("sw","$t0","$fp",(sym.getOffset()), "load lhs addr to T0");
...
class PostDecStmtNode extends StmtNode {
    ...
    public void CodeGen(String r)
    {
    	...
    	Codegen.genPush("$t0");
    	//TODO
    	Codegen.generateIndexed("lw","$t0","$sp",4,"Postdec--"); 
    	SemSym sym = ((IdNode)myExp).sym();
    	if(sym.isGlobal())
    		Codegen.generate("sw","$t0", "_"+((IdNode)myExp).name());
    	else
    		Codegen.generateIndexed("sw","$t0","$fp",(sym.getOffset()), "load lhs addr to T0" + sym.getOffset());
...
class ReadStmtNode extends StmtNode {
    ...
    public void CodeGen(String r)
    {
    	...
    	Codegen.generate("syscall");
    	Codegen.genPush("$v0");
    	Codegen.generateIndexed("lw","$t0","$sp",4,"read--"); 
    	SemSym sym = ((IdNode)myExp).sym();
    	if(sym.isGlobal())
    		Codegen.generate("sw","$t0", "_"+((IdNode)myExp).name());
    	else
...
class WriteStmtNode extends StmtNode {
    ...
    public void CodeGen(String r)
    {
    	myExp.CodeGen();
    	Codegen.genPop("$a0");
    	if(wType.isBoolType()||wType.isIntType())
    		Codegen.generate("li","$v0","1");
    	else if(wType.isStringType())
...
class IfStmtNode extends StmtNode {
    ...
    public void CodeGen(String r)
    {
    	String Jlabel = Codegen.nextLabel();
    	myExp.CodeGen();
    	Codegen.genPop("$t0");
    	Codegen.generate("beq", "$t0", "0", Jlabel);
    	myStmtList.CodeGen(r);
    	Codegen.genLabel(Jlabel);
...
class IfElseStmtNode extends StmtNode {
	...
	myExp.CodeGen();
	Codegen.genPop("$t0");
	Codegen.generate("beq", "$t0", "0", elseLab);

	myThenStmtList.CodeGen(elseLab);

	Codegen.generate("b", doneLab);
	Codegen.genLabel(elseLab);
	myElseStmtList.CodeGen(elseLab);
...
class WhileStmtNode extends StmtNode {
	...
	myExp.CodeGen();
	Codegen.genPop("$t0");
	Codegen.generate("beq", "$t0", "0", doneLab);
	myStmtList.CodeGen(doneLab);
...
class ReturnStmtNode extends StmtNode {
    ...
    public void CodeGen(String r)
    {
        ...
        else
        {
            myExp.CodeGen();
            Codegen.genPop("$t0");
	    Codegen.generate("move","$v0","$t0");
            Codegen.generate("j", r);
        }
    }
    // 1 kid
...
class StringLitNode extends ExpNode {
    ...
    public void CodeGen()
    {
		...
		Codegen.generate(".data");
		Codegen.generateLabeled(stringLab, ".asciiz " + myStrVal, "" );
		Codegen.generate(".text");
...
class IdNode extends ExpNode {
    ...
    public void CodeGen()
    {
		...
		if(mySym.isGlobal())
		{
		    Codegen.generateWithComment("lw", "load Gvar to t0", "$t0", "_"+ myStrVal) ;
		}else{
		    Codegen.generateIndexed("lw", "$t0", "$fp", mySym.getOffset(), "load local var");
		    System.out.println("here is " + mySym.getOffset() + " " + mySym.toString());
		}
		Codegen.genPush("$t0");
    }
...
class AssignNode extends ExpNode {
    ...
    public void CodeGen(){
		myExp.CodeGen();
		Codegen.generateIndexed("lw", "$t0", "$sp", 4, "local");
		...
		if(myLhs instanceof IdNode)
		{
		    ...
		    {
				Codegen.generateIndexed("sw", "$t0", "$fp", ((IdNode)myLhs).sym().getOffset());
		    }
		}
		Codegen.genPop("$t0");
...
class OrNode extends LogicalExpNode {
    ...
    public void CodeGen(){
		...
		String trueLab = Codegen.nextLabel();
		String doneLab = Codegen.nextLabel();
		Codegen.generate("beq", "$t0", "1", trueLab); //jump on true
...
class EqualsNode extends EqualityExpNode {
	...
	if(myExp1 instanceof StringLitNode && myExp2 instanceof StringLitNode){
	    if(((StringLitNode)myExp1).toString().equals(((StringLitNode)myExp2).toString())){
		Codegen.generate("li","$t0",1);
		Codegen.genPush("$t0");
	    }else{
		Codegen.generate("li","$t0",0);
